...
internal static class ConfigCommand
{
    ...
    {
        Console.Write("Agent Skill Store URL: ");
    ...
    private static int Set(ParsedArgs args)
    {
        ...
        {
            ConsoleOutput.WriteError("Usage: skillstore config set <key> <value>");
    ...
    private static int Show()
    {
        ...
        if (config is null)
        {
            ...
            ConsoleOutput.WriteInfo($"Run 'skillstore config init' to create one at {ConfigResolver.ConfigFilePath}");
            return 0;
        }
        ...
        var envUrl = Environment.GetEnvironmentVariable("AGENT_SKILL_STORE_URL");
        var envKey = Environment.GetEnvironmentVariable("AGENT_SKILL_STORE_API_KEY");
        if (envUrl is not null)
            ConsoleOutput.WriteWarning($"AGENT_SKILL_STORE_URL env var set (overrides config): {envUrl}");
        if (envKey is not null)
            ConsoleOutput.WriteWarning($"AGENT_SKILL_STORE_API_KEY env var set (overrides config)");
    ...
    {
        Console.WriteLine("Usage: skillstore config <subcommand>");
        Console.WriteLine();
