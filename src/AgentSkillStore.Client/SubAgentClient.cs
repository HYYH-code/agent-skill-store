...
public sealed partial class AgentSkillStoreClient
{
    public async Task<IReadOnlyList<SubAgentSummary>> ListSubAgentsAsync(CancellationToken ct = default)
    {
        var result = await _httpClient.GetFromJsonAsync(
            Api("subagents"),
            AgentSkillStoreClientJsonContext.Default.IReadOnlyListSubAgentSummary,
            ct);
        return result ?? [];
    }
        ...
        CancellationToken ct = default)
    {
        var result = await _httpClient.GetFromJsonAsync(
            ...
            AgentSkillStoreClientJsonContext.Default.IReadOnlyListSubAgentVersionSummary,
            ct);
        return result ?? [];
    }
        ...
        CancellationToken ct = default)
    {
        return await _httpClient.GetFromJsonAsync(
            ...
            AgentSkillStoreClientJsonContext.Default.SubAgentVersionSummary,
            ct);
    }
        ...
        CancellationToken ct = default)
    {
        ...
        return (await response.Content.ReadFromJsonAsync(
            AgentSkillStoreClientJsonContext.Default.SubAgentUploadResponse,
            ct))!;
    }
