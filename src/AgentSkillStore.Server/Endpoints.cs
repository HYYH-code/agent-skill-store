...
public static class Endpoints
{
    ...
    public static WebApplication MapAgentSkillStoreEndpoints(this WebApplication app)
    {
        app.MapDiscoveryEndpoints();
        app.MapManifestEndpoints();
        app.MapV1Api();
        app.MapEnterpriseApi();
    ...
    private static void MapAppInfoEndpoints(this WebApplication app)
    {
        app.MapGet("/api/v1/info", () =>
        {
            ...
            }, AgentSkillStoreJsonContext.Default.AppInfoResponse);
        });
    }
    ...
    private static void MapDiscoveryEndpoints(this WebApplication app)
    {
            ...
            CancellationToken ct) =>
        {
            ...
            return Results.Json(index, AgentSkillStoreJsonContext.Default.RfcSkillIndex);
        });
    }
    ...
    private static void MapManifestEndpoints(this WebApplication app)
    {
            ...
            CancellationToken ct) =>
        {
            ...
            return Results.Json(manifest, AgentSkillStoreJsonContext.Default.NativeRootManifest);
        });
            ...
            CancellationToken ct) =>
        {
            ...
            return Results.Json(index, AgentSkillStoreJsonContext.Default.NativeSkillCollectionIndex);
        });
            ...
            CancellationToken ct) =>
        {
            ...
            return skillPage is null
                ? Results.NotFound()
                : Results.Json(skillPage, AgentSkillStoreJsonContext.Default.NativeSkillCollectionPage);
        });
            ...
            CancellationToken ct) =>
        {
            ...
            return skill is null
                ? Results.NotFound()
                : Results.Json(skill, AgentSkillStoreJsonContext.Default.NativeSkillIdentityIndex);
        });
            ...
            CancellationToken ct) =>
        {
            ...
            return skillVersion is null
                ? Results.NotFound()
                : Results.Json(skillVersion, AgentSkillStoreJsonContext.Default.NativeSkillVersionDetail);
        });
            ...
            CancellationToken ct) =>
        {
            ...
            return Results.Json(index, AgentSkillStoreJsonContext.Default.NativeSubAgentCollectionIndex);
        });
            ...
            CancellationToken ct) =>
        {
            ...
            return subAgentPage is null
                ? Results.NotFound()
                : Results.Json(subAgentPage, AgentSkillStoreJsonContext.Default.NativeSubAgentCollectionPage);
        });
            ...
            CancellationToken ct) =>
        {
            ...
            return subAgent is null
                ? Results.NotFound()
                : Results.Json(subAgent, AgentSkillStoreJsonContext.Default.NativeSubAgentIdentityIndex);
        });
            ...
            CancellationToken ct) =>
        {
            ...
            return subAgentVersion is null
                ? Results.NotFound()
                : Results.Json(subAgentVersion, AgentSkillStoreJsonContext.Default.NativeSubAgentVersionDetail);
        });
    }
    ...
    private static void MapSkillEndpoints(this WebApplication app)
    {
        ...
        skills.MapGet("/{name}/{version}/SKILL.md", DownloadSkillMd)
            .AddEndpointFilter<InstallableVersionEndpointFilter>();
        ...
        skills.MapPost("/", UploadSkill).DisableAntiforgery()
            ...
            .AddEndpointFilter<SkillSubmitScopeEndpointFilter>()
            .AddEndpointFilter<DirectPublishEndpointFilter>();
    ...
    private static void MapSubAgentEndpoints(this WebApplication app)
    {
        ...
        subagents.MapPost("/", UploadSubAgent).DisableAntiforgery()
            .AddEndpointFilter<ApiKeyEndpointFilter>()
            .AddEndpointFilter<SkillSubmitScopeEndpointFilter>();
        subagents.MapDelete("/{name}/{version}", DeleteSubAgentVersion)
            .AddEndpointFilter<ApiKeyEndpointFilter>()
            .AddEndpointFilter<SkillSubmitScopeEndpointFilter>();
    }
    ...
    private static void MapHealthEndpoints(this WebApplication app)
    {
        app.MapGet("/health", () => Results.Json(
            ...
            AgentSkillStoreJsonContext.Default.HealthResponse));
    }
        ...
        CancellationToken ct)
    {
        ...
        return Results.Json(summaries, AgentSkillStoreJsonContext.Default.IReadOnlyListSkillSummary);
    }
        ...
        CancellationToken ct)
    {
        ...
        var statuses = await enterpriseRepository.GetVersionStatusesAsync(skill.Name, ct);
        var summaries = versions
            .Select(v => ToSkillVersionSummary(skill.Name, v, v.FileCount, GetStatus(v.Version, statuses)))
            .ToList();

        return Results.Json(summaries, AgentSkillStoreJsonContext.Default.IReadOnlyListSkillVersionSummary);
    }
        ...
        CancellationToken ct)
    {
        ...
        var summary = ToSkillVersionSummary(
            skill.Name,
            skillVersion,
            files.Count,
            GetStatus(skillVersion.Version, statuses));

        return Results.Json(summary, AgentSkillStoreJsonContext.Default.SkillVersionSummary);
    }
        ...
        CancellationToken ct)
    {
        ...
        return Results.Json(summaries, AgentSkillStoreJsonContext.Default.IReadOnlyListSkillResourceSummary);
    }
        ...
        out ErrorResponse? error)
    {
        ...
        try
        {
            entries = JsonSerializer.Deserialize(
                raw,
                AgentSkillStoreJsonContext.Default.IReadOnlyListSkillResourceUploadMetadata);
        }
    ...
    private static IResult HandleUploadResult(SkillUploadResult result, IConfiguration configuration)
    {
        ...
        var baseUrl = configuration["AgentSkillStore:BaseUrl"]?.TrimEnd('/') ?? "http://localhost:8081";
        ...
        CancellationToken ct)
    {
        ...
        var statuses = await enterpriseRepository.GetVersionStatusesAsync(name, ct);
        var summary = ToSkillVersionSummary(name, latest, latest.FileCount, GetStatus(latest.Version, statuses));

        return Results.Json(summary, AgentSkillStoreJsonContext.Default.SkillVersionSummary);
    }

    private static SkillVersionSummary ToSkillVersionSummary(
        string name,
        ...
        string status) => new()
    {
        ...
        Status = status,
        Installable = IsInstallableStatus(status),
        ArtifactType = version.SkillType,
        ArtifactSha256 = version.ArtifactSha256,
        ArtifactSizeBytes = version.ArtifactSizeBytes,
        ArchiveUrl = version.SkillType == SkillTypes.Archive
            ? $"/api/v1/skills/{Uri.EscapeDataString(name)}/{Uri.EscapeDataString(version.Version)}/archive.zip"
            : null,
    ...
    private static SkillVersionSummary ToSkillVersionSummary(
        string name,
        SkillVersionWithFileCount version,
        int fileCount,
        string status) => new()
    {
        Name = name,
    ...
    private static string GetStatus(string version, IReadOnlyDictionary<string, string> statuses) =>
        statuses.TryGetValue(version, out var status) ? status : "beta";

    private static bool IsInstallableStatus(string status) =>
        status is not ("draft" or "pending" or "revoked");
        ...
        CancellationToken ct)
    {
        if (request.Count == 0)
            return Results.Json(Array.Empty<CheckUpdateResponseItem>(),
                AgentSkillStoreJsonContext.Default.IReadOnlyListCheckUpdateResponseItem);
        ...
        return Results.Json(results, AgentSkillStoreJsonContext.Default.IReadOnlyListCheckUpdateResponseItem);
    }
        ...
        CancellationToken ct)
    {
        ...
        return Results.Json(summaries, AgentSkillStoreJsonContext.Default.IReadOnlyListSubAgentSummary);
    }
        ...
        CancellationToken ct)
    {
        ...
        return Results.Json(summaries, AgentSkillStoreJsonContext.Default.IReadOnlyListSubAgentVersionSummary);
    }
        ...
        CancellationToken ct)
    {
        ...
        return Results.Json(ToSubAgentVersionSummary(subAgent.Name, subAgentVersion), AgentSkillStoreJsonContext.Default.SubAgentVersionSummary);
    }
    ...
    private static async Task<IResult> CreateApiKey(
        ...
        CancellationToken ct)
    {
        var managementGate = RequireKeyManagementScope(context);
        if (managementGate is not null)
        ...
        return Results.Created($"/api/v1/api-keys/{storedKey.Id}", new CreateApiKeyResponse
        {
            ...
            Key = rawKey,
            AgentId = storedKey.AgentId,
            MachineHash = storedKey.MachineHash,
            Scopes = ApiKeyService.ParseScopes(storedKey.Scopes),
        ...
        CancellationToken ct)
    {
        ...
        var summaries = keys.Select(k => new ApiKeySummary
        {
            ...
            Label = k.Label,
            AgentId = k.AgentId,
            MachineHash = k.MachineHash,
            Scopes = ApiKeyService.ParseScopes(k.Scopes),
            CreatedAt = k.CreatedAt,
            ExpiresAt = k.ExpiresAt
        }).ToList();

        return Results.Json(summaries, AgentSkillStoreJsonContext.Default.IReadOnlyListApiKeySummary);
    }
        ...
        CancellationToken ct)
    {
        ...
        if (!deleted)
        {
            ...
            {
                Error = "delete_failed",
                Message = "Cannot delete key. It may not exist or it may be the last unexpired keys:manage key."
            });
        }
    ...
    private static IResult? RequireKeyManagementScope(HttpContext context)
    {
        var hasScope = context.User.FindAll("scope")
            .SelectMany(claim => claim.Value.Split(' ', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
            .Any(scope => scope.Equals("keys:manage", StringComparison.Ordinal));
        ...
        return hasScope
            ...
            : Results.Json(
                ...
                {
                    Error = "forbidden",
                    Message = "API key management requires the keys:manage scope."
                },
