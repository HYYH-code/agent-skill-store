...
{
    private static readonly HashSet<string> AllowedEnterpriseRootFiles = new(StringComparer.OrdinalIgnoreCase)
    {
        "manifest.yaml",
        "CHANGELOG.md",
        "LICENSE"
    ...
    public static bool TryCreate(string? value, [NotNullWhen(true)] out ResourcePath? result)
    {
        ...
        // Standard resources use subdirectories. Enterprise governance files have a small,
        // explicit root allow-list so SKILL.md remains the only executable entry document.
        if (segments.Length < 2 && !AllowedEnterpriseRootFiles.Contains(normalized))
            return false;
