...
public sealed class ClientForwardCompatTests
{
    ...
    private static AgentSkillStoreClient CreateClient(string json)
    {
        var httpClient = new HttpClient(new CannedJsonHandler(json))
        {
            BaseAddress = new Uri("http://localhost/")
        };
        return new AgentSkillStoreClient(httpClient);
    }
    ...
    public async Task GetManifest_WithUnknownLinkAndTopLevelField_IsTolerated()
    {
        const string json = """
            {
              "$schema": "https://agent-skill-store.dev/manifest/v1",
              "apiVersion": "v1",
