...
public sealed class CliArgsParserTests
{
    ...
    [Fact]
    public void Parse_ApiKeyGenerate()
    {
        var result = CliArgsParser.Parse(["api-key", "generate"]);

        Assert.Equal("api-key", result.Command);
        Assert.Equal("generate", result.SubCommand);
    }

    [Fact]
    public void Parse_ApiKeyCreate_WithAgentIdentity()
    {
        var result = CliArgsParser.Parse([
            "api-key", "create",
            "--label", "Agent A",
            "--agent-id", "skillstore-agent-a",
            "--machine-hash", "abc123",
            "--scope", "skills:read",
            "--scope", "skills:submit"
        ]);
        ...
        Assert.Equal("api-key", result.Command);
        Assert.Equal("create", result.SubCommand);
        Assert.Equal("skillstore-agent-a", result.AgentId);
        Assert.Equal("abc123", result.MachineHash);
        Assert.Equal(["skills:read", "skills:submit"], result.Scopes);
    }

    [Fact]
    public void Parse_SubmitCommand_WithGovernanceOptions()
    {
        var result = CliArgsParser.Parse([
            "submit", "./skill.zip",
            "--name", "vehicle-log-diagnosis",
            "--display-name", "车端日志诊断工装",
            "--namespace", "connected",
            "--version", "1.0.0",
            "--owner", "Platform Team",
            "--risk-level", "medium",
        ...
        Assert.Equal("./skill.zip", result.Positional[0]);
        Assert.Equal("vehicle-log-diagnosis", result.Name);
        Assert.Equal("车端日志诊断工装", result.DisplayName);
        Assert.Equal("connected", result.Namespace);
        Assert.Equal("1.0.0", result.VersionOverride);
        Assert.Equal("Platform Team", result.Owner);
        Assert.Equal("medium", result.RiskLevel);
        Assert.Equal("deny-all", result.NetworkPolicy);
        Assert.Equal("Initial release", result.Changelog);
        Assert.Equal("json", result.OutputFormat);
    }
    ...
    [Fact]
    public void Parse_InstallCommand_WithTargetAndRoot()
    {
        var result = CliArgsParser.Parse([
        ...
        Assert.Equal("install", result.Command);
        Assert.Equal("my-skill", result.Positional[0]);
    ...
    [Fact]
    public void Parse_LoginAliasesAndNonStableFlag()
    {
        var result = CliArgsParser.Parse([
            "login",
            "--server", "https://skillstore.example.com",
            "--api-key", "sk-test",
            "--agent", "pi",
            "--allow-non-stable"
        ]);

        Assert.Equal("login", result.Command);
        Assert.Equal("https://skillstore.example.com", result.ServerUrl);
        Assert.Equal("sk-test", result.ApiKey);
