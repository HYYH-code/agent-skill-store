...
public sealed class PublishSubAgentsCommandTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"agent-skill-store-subagents-publish-{Guid.NewGuid():N}");
    ...
    private static HttpResponseMessage JsonResponse(string name, string version)
    {
        ...
        var json = JsonSerializer.Serialize(response, AgentSkillStoreClientJsonContext.Default.SubAgentUploadResponse);
