...
public sealed class DownloadSubAgentCommandTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"agent-skill-store-subagent-download-{Guid.NewGuid():N}");
    ...
    private static AgentSkillStoreClient CreateClient(HttpMessageHandler handler)
    {
        return new AgentSkillStoreClient(new HttpClient(handler)
        {
    ...
    {
        var json = JsonSerializer.Serialize(detail, AgentSkillStoreClientJsonContext.Default.NativeSubAgentVersionDetail);
