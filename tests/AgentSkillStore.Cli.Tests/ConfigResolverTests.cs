...
public sealed class ConfigResolverTests
{
    ...
    {
        var originalUrl = Environment.GetEnvironmentVariable("AGENT_SKILL_STORE_URL");
        var originalKey = Environment.GetEnvironmentVariable("AGENT_SKILL_STORE_API_KEY");

        try
        {
            Environment.SetEnvironmentVariable("AGENT_SKILL_STORE_URL", null);
            Environment.SetEnvironmentVariable("AGENT_SKILL_STORE_API_KEY", null);
        ...
        {
            Environment.SetEnvironmentVariable("AGENT_SKILL_STORE_URL", originalUrl);
            Environment.SetEnvironmentVariable("AGENT_SKILL_STORE_API_KEY", originalKey);
        }
    }

    [Fact]
    public void Resolve_AgentSkillStoreEnvVars_UsedWhenNoFlags()
    {
        var originalUrl = Environment.GetEnvironmentVariable("AGENT_SKILL_STORE_URL");
        var originalKey = Environment.GetEnvironmentVariable("AGENT_SKILL_STORE_API_KEY");
        ...
        try
        {
            Environment.SetEnvironmentVariable("AGENT_SKILL_STORE_URL", "https://env.example.com");
            Environment.SetEnvironmentVariable("AGENT_SKILL_STORE_API_KEY", "env-key");
