...
Status: Implemented. `/manifest.json` is the root discovery endpoint for Agent Skill Store-aware clients.

The native manifest is Agent Skill Store's versioned, HATEOAS-style sync feed. It must not replace the Cloudflare Agent Skills Discovery RFC feed (`/.well-known/agent-skills/index.json`). Instead, it provides a linked catalog for Agent Skill Store-aware clients that need version negotiation, search, version history, sub-agent definitions, and additional metadata.

## Goals

- Add `/manifest.json` as a native entry point for Agent Skill Store-aware clients.
...
```json
{
  "$schema": "https://schemas.agent-skill-store.dev/agent-skill-store/manifest/0.1.0",
  "apiVersion": "v1",
  "versions": {
    "v1": {
...
Resourceful skill artifacts use Agent Skill Store's canonical `.zip` archive route, `/api/v1/skills/{skillName}/{version}/archive.zip`.
...
- Agent Skill Store may install server-synced skills and sub-agents under managed directories inside its configured home directory.
- A non-Agent Skill Store client may sync the same artifacts into its own skills, commands, agents, plugins, or cache directories.
...
## Non-Agent Skill Store Client Sync

Non-Agent Skill Store clients should consume the native manifest through an adapter layer rather than requiring Agent Skill Store to publish client-specific feeds.

The recommended split is:

- Agent Skill Store publishes canonical, verified artifacts.
- `AgentSkillStore.Client` fetches manifests and artifact bytes without choosing local paths or target client formats.
...
This is the same contract Agent Skill Store should use for skills and sub-agents: sync tooling can fetch, verify, compare, and stage artifacts, but the consuming client decides how those artifacts become local runtime configuration.
...
If the target client can consume Agent Skill Store-style markdown directly, the adapter can install `agent-md` without conversion. If the target client has a different format, the adapter owns the mapping.
...
Read access may be open for public registries. Private registries may require the same bearer API key already used by Agent Skill Store write endpoints and Agent Skill Store feed configuration.
