...
Status: current Agent Skill Store file format, target Agent Skill Store publication model.

This document defines the sub-agent definition format Agent Skill Store should publish and the Agent Skill Store runtime already understands. Sub-agents are not AgentSkills.io skills and must not be listed in the Cloudflare RFC skill feed.

## Goals

- Preserve the existing Agent Skill Store sub-agent markdown format.
...
Agent Skill Store currently loads sub-agent definitions from one markdown file per agent:

```text
~/.agent-skill-store/agents/
...
Current Agent Skill Store-supported fields:
...
Unknown fields are ignored by the current Agent Skill Store loader. Agent Skill Store publication may read additional metadata without affecting Agent Skill Store runtime behavior.

## Versioning

Agent Skill Store local runtime does not require a sub-agent version today.

Agent Skill Store publication does require a version for sync, caching, and rollback. The initial publication flow should accept a version out of band through the CLI, matching skill upload behavior. A future CLI may also accept a compatible frontmatter extension such as:
...
Until Agent Skill Store runtime needs local version awareness, version metadata should remain a Agent Skill Store packaging concern.

## Runtime Semantics

Sub-agents are specialist workers spawned by a parent Agent Skill Store session.
...
Sub-agents should not declare required skills in the first version. If a sub-agent needs a skill's instructions, encode that operational guidance in the sub-agent prompt or use a future Agent Skill Store runtime field after there is a concrete loading requirement.

## Agent Skill Store Artifact Shape
...
| `version` | Agent Skill Store publication version. |
...
Agent Skill Store does not define where sub-agent artifacts are installed after download. It publishes versioned `agent-md` artifacts and native manifest metadata; each client maps those artifacts into its own local configuration model.

Agent Skill Store sync should install downloaded sub-agent definitions into a managed server-feed subdirectory under the Agent Skill Store agents area rather than overwriting user-authored files directly.

The exact local path is a Agent Skill Store implementation detail, not a Agent Skill Store protocol requirement. Agent Skill Store's layout should preserve these properties:

- User-authored `~/.agent-skill-store/agents/*.md` files remain editable.
...
Non-Agent Skill Store clients can use the same server artifacts by configuring their own sync destination. They should preserve equivalent ownership boundaries between user-authored files and server-synced files.

## Client Adapters

Non-Agent Skill Store clients should sync sub-agents through an adapter that understands that client's local agent format.

The canonical Agent Skill Store artifact is `agent-md`: YAML frontmatter plus a markdown system prompt body. An adapter may either install this file directly or translate it into the target client's native format.

Sub-agent sync should follow the same unopinionated model as skill sync. Shared tooling should fetch and verify artifacts, compare local state, and hand the result to a destination adapter. It should not assume Agent Skill Store's `~/.agent-skill-store/agents` layout, OpenCode's layout, Claude Code's layout, or any other target layout.
...
This keeps Agent Skill Store portable: it publishes one sub-agent artifact model, while clients own installation and format translation.
